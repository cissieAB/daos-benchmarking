...
\begin{column}{\colwidth}
  ...
  \begin{block}{DAOS Storage System}
    ...
    (logical collections of objects) may be created. Objects (key-value stores). 
    Keys have a distribution key (dkey, determining the storage target) 
    and attribute key (akey), with values being either single atomic values or arrays.

    DAOS can be accessed via the native DAOS library libdaos, 
    libpil4dfs interception library (read/write/metadata), 
    libioil interception library (read/write), 

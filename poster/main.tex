...
\begin{column}{\colwidth}
  ...
  \begin{block}{DAOS Storage System}
    ...
    Distributed Asynchronous Object Storage (DAOS) is a high performance storage system using key-array-value store allowing end-to-end user space operation, asynchronous I/O operation, and non-blocking I/O.
    
    DAOS is provisioned in pools (storage allocations), wherein containers (logical collections of objects)
    may be created. Objects (key-value stores). Keys have a distribution key (dkey, determining the storage target) 
    and attribute key (akey), with values being either single atomic values or arrays.

    DAOS accessed via the native DAOS library libdaos, libpil4dfs interception library (read/write/metadata), libioil interception library (read/write), and mounting a container via DFUSE.
    ...
    \begin{figure}
      \centering
        \includegraphics[width=\colwidth]{diagrams/data_heirarchy.pdf}
    ...
    \begin{figure} 
      \centering
        ...
        \label{fig:posix}
      \caption{DAOS storage system architecture \cite{xmei_sc24}}
    \end{figure}
  ...
  \begin{block}{Aurora} 
    ...
    \begin{itemize}
      ...
      \item Nodes have 8 HPE Slingshot-11 NICs, using dragonfly topology
      \item Aurora compute nodes use two Intel Xeon CPU Max Series, with 52 cores each. For ior and mdtest we bind to 48 cores per node.
...
\begin{column}{\colwidth}
  ...
  \begin{block}{fio Bandwidth}
    ...
    \begin{figure}
      ...
      \caption{For one client node with 16 jobs with 16 I/O units kept in flight, 
      the bandwidth vs block size for fio using the pvsync I/O engine. 
      Block size in fio is the size of an I/O unit. This desmonstrates the 
    ...
    \begin{figure} 
      ...
      \caption{ For one client node with 16 jobs with 16 I/O units kept in flight, 
      the proportions of overall bandwidth spent reading and writing across 
      different workloads. The yellow line indicates the expected ratio of bandwidth 
      proportions. Buffered I/O will use an intermediate buffer between applications 
    ...
    We did not observe a large difference in performance between 
    random and sequential access patterns. 
    Additionally, for dfuse access, changing the number of fio jobs 
    (additional threads cloning the original thread) or I/O depth 
    (number of I/O units to keep in flight) has little effect on performance, 
    while the number of jobs using the native DFS API increases latency 
    (Fig.~\ref{fig:ior_latency}) and bandwidth 
    (Fig.~\ref{fig:ior_bw-vs-tasks_hue-nodes_2M},\ref{fig:ior_bw-vs-tasks_hue-nodes_16M}).
  ...
  \begin{block}{IOR Latency}
    \begin{figure} 
      ...
      \caption{ For a transfer size of 1 MiB, the ior latency vs number of tasks. 

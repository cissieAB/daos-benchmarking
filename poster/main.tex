...
\begin{column}{\colwidth}
  ...
  \begin{block}{DAOS Storage System}
    ...
    \begin{figure}
      ...
      \caption{Diagram of DAOS pool data heirarchy \cite{daos_guide}}
    ...
    \begin{figure}
      \centering
        ...
        \label{fig:posix}
      \caption{Diagram of DAOS storage system communication flow from applications to DAOS storage engines \cite{daos_guide}}
    \end{figure}
  ...
  \begin{block}{Aurora Architecture} 
    ...
    \begin{figure}
      ...
      \caption{Diagram of DAOS storage system architecture on Aurora from Argonne National Lab \cite{argonne_daos}}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{fio Bandwidth}
    ...
    Additionally, for dfuse access, changing the number of fio jobs (additional threads cloning the original thread) or I/O depth (number of I/O units to keep in flight) has little effect on performance, while the number of jobs using the native DFS API increases latency (Fig.~\ref{fig:ior_latency}) and bandwidth (Fig.~\ref{fig:ior_bw-vs-tasks_hue-nodes-2M},\ref{fig:ior_bw-vs-tasks_hue-nodes-16M}).
...
\begin{column}{\colwidth}
  ...
  \begin{block}{IOR Bandwidth}
    ...
    \begin{figure}
      ...
      \label{fig:ior_bw-vs-tasks_hue-nodes-2M}
    \end{figure}
    \begin{figure}
      \centering
